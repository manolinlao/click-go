...
import { formatImport, getFilledString } from '../../helpers/utils';
...
export const ClickGo002ChangePlz = () => {

  const navigate = useNavigate();
  const dispatch = useDispatch();

  const clickgoData = useSelector(state=>state.clickgoData);  
  const minPeriodos = clickgoData.datosAdmisionActivo.minPeriodos;
  const maxPeriodos = clickgoData.datosAdmisionActivo.maxPeriodos;
  ...
  const objScreen = {
    jliTitle: '¿En cuántos meses quieres pagar el préstamo?',
    jliTxtSubForm: '(~ a ~ meses)',
  ...
  const goNext = ()=>{
    ...
    if(tmpValue<minPeriodos || tmpValue>maxPeriodos){  
      let msgNok = `El plazo de ${tmpValue} meses supera el máximo permitido`;
      if(tmpValue<minPeriodos){
        msgNok = `El plazo de ${tmpValue} meses es inferior al mínimo permitido`;
      }
  ...
  let txtSubForm = '';
  txtSubForm =  getFilledString(objScreen.jliTxtSubForm,formatImport(minPeriodos),formatImport(maxPeriodos));
  ...
  return (
    <div className='screen-container'>
      ...
      <div style={{marginTop:'20px',textAlign:'center'}}>
        <FormImport value={ formInputValue }/>       
